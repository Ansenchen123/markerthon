...
Daily sold and recovered reports are append-only CSV logs. Each successful QR creation appends a `sold` row, and each successful scan return appends a `recovered` row to that day's CSV file. Merchant stats APIs aggregate these CSV logs by date, always include all container types, and return one row per requested day; government daily APIs aggregate the same CSV logs.
